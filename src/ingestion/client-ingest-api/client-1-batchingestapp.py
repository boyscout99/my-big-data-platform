...
def cassandra_connection():
	...
	session.execute("""
		CREATE KEYSPACE IF NOT EXISTS networkdata_client_1
		WITH REPLICATION =
		{ 'class' : 'SimpleStrategy', 'replication_factor' : 2 };
	""")
	time.sleep(5)
	session.set_keyspace('networkdata_client_1')
	#session.execute("""USE networkdata;""")
	session.execute("""
		CREATE TABLE IF NOT EXISTS device_by_onuid_client_1 (
		time text,
		provincecode text,
		ifindex text,
		onuid text,  
...
def ingest_file(path_to_csv):
	...
	"""
	host = 'localhost'
	keyspace_name = 'networkdata_client_1'
	table_name = 'device_by_onuid_client_1'
...
if __name__ == "__main__":
	...
	print('Reading data to process for client-1 ...')
	path_to_csv = './client-ingest-api/client-1-staging-input-directory/unproc-data-client-1.csv'
	path_to_proc_csv = './client-ingest-api/client-1-staging-input-directory/processed-data/proc-data-client-1.csv'
	read_file(path_to_csv, path_to_proc_csv)
	print('Data wrangling performed. New file created.')
	ingest_file(path_to_proc_csv)
	print('Ingested processed data for client-1.')
