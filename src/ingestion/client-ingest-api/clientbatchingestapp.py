...
def cassandra_connection():
	...
	session.execute("""
		CREATE KEYSPACE IF NOT EXISTS networkdata
		WITH REPLICATION =
		{ 'class' : 'SimpleStrategy', 'replication_factor' : 2 };
	""")
	time.sleep(5)
	session.set_keyspace('networkdata')
	#session.execute("""USE networkdata;""")
	session.execute("""
		CREATE TABLE IF NOT EXISTS device_by_onuid (
		time text,
		provincecode text,
		ifindex text,
		onuid text,  
...
def ingest_file(path_to_csv):
	...
	"""
	host = 'localhost'
	keyspace_name = 'networkdata'
	table_name = 'device_by_onuid'
...
if __name__ == "__main__":
	...
	print('Reading data to process ...')
	path_to_csv = './client-ingest-api/client-staging-input-directory/unproc-data.csv'
	path_to_proc_csv = './client-ingest-api/client-staging-input-directory/processed-data/proc-data.csv'
	read_file(path_to_csv, path_to_proc_csv)
	print('Data wrangling performed. New file created.')
	ingest_file(path_to_proc_csv)
	print('Ingested processed data.')
